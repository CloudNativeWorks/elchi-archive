...
backend::install_binaries() {
  ...
  for v in "${variants[@]}"; do
    ...
    # .github/workflows/build-elchi-backend.yml so this unauthenticated
    # curl works even though CloudNativeWorks/elchi-backend is private.
    # The release is re-tagged elchi-backend-<release>; the asset keeps
    # its upstream name (the full variant tag), so backend_asset_basename
    # is unchanged.
    url="https://github.com/CloudNativeWorks/elchi-archive/releases/download/elchi-backend-${release}/${asset}"
    sha_url="${url}.sha256"
