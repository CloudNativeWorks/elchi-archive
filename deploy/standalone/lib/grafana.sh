...
grafana::_install_debian() {
  ...
  apt-get -o DPkg::Lock::Timeout=600 install -y -qq apt-transport-https software-properties-common gnupg curl
  ...
  preflight::wait_apt_lock 600 || true
  apt-get -o DPkg::Lock::Timeout=600 install -y -qq grafana
}
