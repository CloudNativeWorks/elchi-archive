...
sudo bash deploy/standalone/install.sh \
  ...
  --backend-version=elchi-v1.6.16-v0.14.0-envoy1.39.0 \
  --ui-version=v1.5.23 \
  --envoy-version=v1.39.0 \
...
|------------------|----------------------------------|---------|
| elchi-backend    | `ELCHI_DEFAULT_BACKEND_VARIANTS` | `elchi-v1.6.16-v0.14.0-envoy1.39.0` |
| elchi UI         | `ELCHI_DEFAULT_UI_VERSION`       | `v1.5.23` |
| Envoy            | `ELCHI_DEFAULT_ENVOY_VERSION`    | `v1.39.0` |
...
sudo bash deploy/standalone/upgrade.sh --ui-version=v1.5.23
...
sudo bash deploy/standalone/upgrade.sh \
  --add-backend-version=elchi-v1.6.16-v0.14.0-envoy1.39.1
...
sudo bash deploy/standalone/upgrade.sh --ui-version=v1.5.23 --upgrade-os
```
...
| Image registries (`jhonbrownn/elchi*` on DockerHub) | Everything served from `CloudNativeWorks/elchi-archive` releases (this public repo): backend variant binaries, UI bundle, envoy, coredns, collector — all mirrored from their private source repos by the `.github/workflows/build-*.yml` mirror jobs. Upstream apt/yum for mongo/grafana/otel/vm | DockerHub images aren't directly usable on bare metal. Source repos (elchi-backend, elchi, elchi-collector, …) are private, so install-time downloads must hit the single public mirror — the installer fetches every elchi binary with an UNAUTHENTICATED curl |
...
| ClickHouse resources via k8s pod limits | Host-proportional systemd drop-in: `MemoryMax` = 40% RAM (floor 1G), `MemoryHigh` = 90% of that, `CPUQuota` = half the cores (floor 100%). Override: `ELCHI_CLICKHOUSE_MEMORY_MAX` / `ELCHI_CLICKHOUSE_CPU_QUOTA` | ClickHouse defaults assume a dedicated box (90% of RAM, 16 merge threads); an elchi node co-locates it with mongod/Envoy/backend/VM/Grafana. ClickHouse is cgroup-aware, so the systemd cap doubles as its internal memory budget |
...
2. **`x-target-cluster` format** ✓ — controller IDs are bare hostnames
   ...
   full semver (`1.38.3`, `1.39.0`). K8s detection uses the
