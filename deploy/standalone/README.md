...
|------------------|----------------------------------|---------|
| elchi-backend    | `ELCHI_DEFAULT_BACKEND_VARIANTS` | `elchi-v1.6.15-v0.14.0-envoy1.39.0` |
...
sudo bash deploy/standalone/upgrade.sh --ui-version=v1.5.21
...
sudo bash deploy/standalone/upgrade.sh --ui-version=v1.5.21 --upgrade-os
```
...
| Helm chart's UI `image.tag: v1.0.0` | Default `--ui-version=v1.5.21` | Helm chart pin is older than the latest UI release; bare-metal default tracks the current release |
